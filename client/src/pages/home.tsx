...
export default function HomePage() {
  ...
  // Seed demo case once for brand-new users
  useEffect(() => {
    if (!user?.id || isLoading) return;
    const key = `demo_seeded_${user.id}`;
    if (localStorage.getItem(key)) return;
    localStorage.setItem(key, "1");
    apiRequest("POST", "/api/cases/seed-demo")
      .then(() => queryClient.invalidateQueries({ queryKey: ["cases", user.id] }))
      .catch(() => {});
  }, [user?.id, isLoading]);
      ...
      <div className="px-6 space-y-8 pb-8">
        <div className="grid grid-cols-2 gap-5">
          <Link href="/record" className="relative overflow-hidden flex flex-col items-center justify-center p-6 rounded-3xl bg-gradient-to-br from-primary to-blue-600 text-white shadow-xl shadow-blue-500/30 hover:shadow-2xl hover:shadow-blue-500/40 hover:-translate-y-1 transition-all duration-300 group" data-testid="link-new-session">
            ...
            <span className="font-bold text-lg tracking-tight">{t("home.new_session")}</span>
            <span className="text-xs text-blue-100 mt-1 font-medium">{t("home.auto_scribe")}</span>
          </Link>
          ...
          <Link href="/cases" className="relative overflow-hidden flex flex-col items-center justify-center p-6 rounded-3xl bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 shadow-xl shadow-slate-200/50 dark:shadow-black/20 hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 group" data-testid="link-cases">
            ...
            <span className="font-bold text-lg tracking-tight text-foreground">{t("home.all_cases")}</span>
            <span className="text-xs text-muted-foreground mt-1 font-medium">{cases.length} {t("home.records")}</span>
          </Link>
        </div>
        ...
        <Link href="/subscription" className="block group" data-testid="link-subscription">
          <div className="relative overflow-hidden rounded-3xl bg-slate-900 dark:bg-white p-1">
            ...
            <div className="relative bg-slate-950 dark:bg-slate-100 rounded-[20px] p-5 flex items-center justify-between">
              <div>
                 <h3 className="font-bold text-white dark:text-slate-900 text-lg">{t("home.go_pro")}</h3>
                 <p className="text-slate-400 dark:text-slate-600 text-xs mt-1">{t("home.go_pro_sub")}</p>
              </div>
        ...
        <div className="space-y-4">
          <div className="flex justify-between items-end px-1">
            <h2 className="text-xl font-heading font-bold text-foreground">{t("home.recent_cases")}</h2>
            <Link href="/cases" className="text-sm text-primary font-semibold hover:text-primary/80 transition-colors" data-testid="link-view-all">{t("home.view_all")}</Link>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="text-center py-12 bg-slate-50 dark:bg-slate-900 rounded-2xl">
              ...
              <h3 className="font-semibold text-slate-600 dark:text-slate-400 mb-2">{t("home.no_cases")}</h3>
              <p className="text-sm text-slate-400">{t("home.no_cases_sub")}</p>
            </div>
          ) : (
            <div className="space-y-4">
